# =============================================================================
# pretrain_vit.py — Pre-training for MAT-ViT-LWM (Method 1: 32×32 ViT)
#
# This script pretrains the MAT-ViT-LWM hybrid model using masked spatial
# prediction on DeepMIMO wireless channel data.
#
# It mirrors the structure and style of lwm1_1_ca/pretraining.py:
#   • Same DeepMIMO scenarios and data loading pipeline
#   • Same hyperparameters (lr, batch_size, epochs, scheduler…)
#   • Same CLI arguments and logging conventions
...
def parse_args() -> argparse.Namespace:
    ...
    Defaults match those used in lwm1_1_ca/pretraining.py for fair comparison.
    """
    parser = argparse.ArgumentParser(
        description=(
            "Pre-training for MAT-ViT-LWM (Method 1: 32×32 ViT Approach).\n"
            "Uses the same scenarios, hyperparameters, and data loading as "
            "lwm1_1_ca/pretraining.py."
        )
    )
    ...
    parser.add_argument(
        "--dataset-folder", type=str, default=None,
        help="Path to the DeepMIMO scenarios root folder.",
    )
    parser.add_argument(
        "--channels-cache", type=str, default=None,
        help="Optional .npy path for caching the stacked (N,2,H,W) channel array.",
    )
    ...
    parser.add_argument("--snr-db",       type=float, default=None, help="AWGN noise level (dB).")
    parser.add_argument("--mask-ratio",   type=float, default=0.15, help="Spatial mask ratio.")
    ...
    parser.add_argument("--tb-logdir",    type=str, default="runs/mat_vit_lwm_pretraining")
...
def load_channels_ri(
    ...
    Returns a (N, 2, H, W) float32 ndarray.
    """
    ...
    channels = np.concatenate(data_parts, axis=0)            # (N, 1, H, W) complex
    real = channels.real.astype(np.float32)
    imag = channels.imag.astype(np.float32)
    channels_ri = np.concatenate([real, imag], axis=1)       # (N, 2, H, W)
...
def train_epoch(
    ...
    The MATViTLWM model returns (loss, pred_masked, target_masked) directly,
    so we simply use the returned loss for backward.
    ...
    for step, (channels,) in enumerate(dataloader):
        ...
        with autocast_ctx:
            # MATViTLWM returns (loss, pred_masked, target_masked) in training
...
def validate_epoch(
    ...
    """Evaluate on validation (or test) data without gradient updates."""
...
def main():
    ...
    log("=" * 60)
    log("MAT-ViT-LWM (Method 1: 32×32 ViT) — Pre-training")
    log("=" * 60)
