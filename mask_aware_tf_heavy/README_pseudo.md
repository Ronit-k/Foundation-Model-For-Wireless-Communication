...
To counter this, **LayerScale** along with a **Stable Identity Initialization Protocol** is strictly applied across the entire network body:
...
- **Stable Identity Initialization:** Because the attention fusion `1x1` convolution natively breaks the strict identity pathway, the model uses a deterministic `Dirac` delta initialization for the identity token mapping while zeroing out attention mapping. Additionally, residual mappings inside `mlp` and `local_conv` are strictly **Zero-Initialized** at their terminal outputs. 
- **Exploding Gradient Nullification:** Every `gamma` is initialized safely to a `1e-4` scale. Coupled with the Identity Initialization, this essentially compels the deep 6-layer `Heavy_MAT` model to perfectly feign an exact shallow identity function at Epoch 0, eliminating variance explosion while ensuring clean, deep gradient pathways. As convergence aligns smoothly over descending epochs, the model gently trains the `gamma` scales up and the convolution weights to introduce deeper representation fidelity.
