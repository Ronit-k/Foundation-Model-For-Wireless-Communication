# =============================================================================
# mat_pseudo_lwm_ca.py — MAT Pseudo-Grid LWM + Coordinate Attention
#
# Extends MATPseudoLWM (Method 2: 8×16 Pseudo-Grid) with a Coordinate
# Attention (CA) front-end that recalibrates the raw (B,2,H,W) channel
# in the antenna (H) and subcarrier (W) directions before patching.
#
# Data flow:
#   (B,2,32,32) → CoordAtt → flatten → (B,128,16) → mask 15%
#   → Linear(16→64) + PosEmb → (B,128,64) → reshape → (B,64,8,16)
#   → 3-stage ATB → reshape back → (B,128,64) → gather masked
#   → Linear(64→16) → MSE vs raw patches
#
# Inference:
...
from mask_aware_tf.mat_vit_lwm import ATB, WindowMHA

# Local Coordinate Attention module
from .coordatt import CoordAtt


# =============================================================================
# Helpers — LWM-compatible patching (copied from mask_aware_tf.mat_pseudo_lwm)
# =============================================================================
...
# =============================================================================
# MAT Pseudo-Grid LWM + Coordinate Attention Model
# =============================================================================

class MATPseudoLWMWithCA(nn.Module):
    """
    Hybrid MAT–LWM model with Coordinate Attention front-end.

    Extends MATPseudoLWM by inserting a CoordAtt module before LWM patching.
    The CA module recalibrates the raw (B,2,H,W) wireless channel along
    both spatial axes (antenna direction H and subcarrier direction W),
    allowing the ATB backbone to focus on the most informative spatial
    features for masked-token reconstruction.

    Training mode (``gen_raw=False``):
        CoordAtt → Tokenize → mask → embed → 2D reshape → ATB → 1D reshape
        → decode → MSE loss on masked patches.

    Inference mode (``gen_raw=True``):
        CoordAtt → Tokenize → embed → 2D reshape → ATB → embeddings.
    ...
    Args:
        ...
        coordatt_reduction:  CA bottleneck ratio.   Default 32.
    """
    ...
    def __init__(
        ...
        # ── Coordinate Attention: 2-channel in, 2-channel out ────────────
    ...
    def forward(self, channels: torch.Tensor):
        ...
        # ── Step 0: Coordinate Attention — recalibrate spatial features ──
        channels = self.coordatt(channels)                    # (B, 2, 32, 32)
