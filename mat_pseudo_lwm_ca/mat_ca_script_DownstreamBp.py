...
print(
    "---------------------------- training Details ----------------------------\n"
    f"Model: MAT-PSEUDO-LWM-CA (light) ({down_choice})\n"
    f"epochs: {num_epochs}, "
...
# %%
#%% LOAD THE MODEL — MAT Pseudo-Grid LWM + Coordinate Attention
gpu_ids = [0]
...
lwm_model = model
lwm_model.eval()
print(f"Model [MATPseudoLWMWithCA] loaded successfully on {device}")
...
def get_embeddings_mat_ca(mat_model, channels_ri, input_type, device, batch_size=64):
    """Extract embeddings from MAT Pseudo-Grid LWM + CA model."""
    ...
    with torch.no_grad():
        for (batch,) in loader:
            ...
            if input_type == "raw":
                ...
                # Apply CoordAtt first (like the model does internally)
                ca_out = mat_model.coordatt(batch)
                patches = channels_to_patches(ca_out)  # (B, 128, 16)
...
with torch.no_grad():
    dataset = get_embeddings_mat_ca(lwm_model, channels_ri, selected_input_type, device, batch_size=batch_size)
...
# %%
# Mapping for beam prediction input types — MAT Pseudo-Grid LWM + CA uses d_model=64
mapping = {
    'cls_emb': {'input_channels': 1, 'sequence_length': 64},       # CLS dim = 64
    'channel_emb': {'input_channels': 64, 'sequence_length': 128}, # (B, 128, 64) -> [B, L, C]
    'raw': {'input_channels': 16, 'sequence_length': 128}          # (B, 128, 16) raw patches (after CA)
}
...
photo_dir = os.path.join(os.path.dirname(__file__), "photos", f"{photo_prefix}_{down_choice}_{selected_input_type}")
