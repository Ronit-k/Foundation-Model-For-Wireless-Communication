# =============================================================================
# pretrain_pseudo_ca.py — Pre-training for MAT Pseudo-Grid LWM + CA
#
# Pre-trains the MAT Pseudo-Grid LWM + Coordinate Attention hybrid model
# using masked channel modelling (MCM) on DeepMIMO wireless channel data.
#
# Mirrors the structure of mask_aware_tf/pretrain_pseudo.py:
...
#   • Adds CoordAtt front-end trained jointly with the ATB backbone
#
# Output naming format:
#   {batch_size}_{epochs}mat_pseudo_lwm_ca_weights.pth
#   {batch_size}_{epochs}mat_pseudo_lwm_ca_weights.log
#
# Usage (from project root):
#   python -m mat_pseudo_lwm_ca.pretrain_pseudo_ca [options]
#
# Quick start:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pre-training for MAT Pseudo-Grid LWM + Coordinate Attention."
    )
    ...
    parser.add_argument("--coordatt-reduction", type=int, default=32)
    ...
    parser.add_argument("--tb-logdir",    type=str, default="runs/mat_pseudo_lwm_ca_pretraining")
...
# =============================================================================
# Data Loading (same as mask_aware_tf/pretrain_pseudo.py)
# =============================================================================
...
def main():
    ...
    # ── Build save-path and log-file with {batch_size}_{epochs} naming ───
    if args.save_path is None:
        args.save_path = (
            f"mat_pseudo_lwm_ca/"
            f"{args.batch_size}_{args.epochs}mat_pseudo_lwm_ca_weights.pth"
        )
    ...
    model = MATPseudoLWMWithCA(
        gen_raw=False,
        snr_db=args.snr_db,
        mask_ratio=args.mask_ratio,
        coordatt_reduction=args.coordatt_reduction,
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    n_ca_params = sum(p.numel() for p in model.coordatt.parameters())
    print(f"[Model] MATPseudoLWMWithCA created. Parameters: {n_params:,} (CA: {n_ca_params:,})")
    ...
    if log_file is None:
        base = args.save_path or "mat_pseudo_lwm_ca/pretrain_pseudo_ca"
    ...
    log("=" * 60)
    log("MAT Pseudo-Grid LWM + Coordinate Attention — Pre-training")
    log("=" * 60)
    ...
    log(f"CA reduction: {args.coordatt_reduction}")
